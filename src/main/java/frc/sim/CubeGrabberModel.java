...
class CubeGrabberModel {
    ...
    public void update(boolean isDisabled){
        ...
        if(!isDisabled){
            lhSpdCmd = lhMotorCtrl.getSpeed();
            rhSpdCmd = rhMotorCtrl.getSpeed();
        }

        lhIntakeMotor.update(12.5, -1.0 * lhSpdCmd, 0.0); //mechanically inverted
        rhIntakeMotor.update(12.5, rhSpdCmd, 0.0);
    }
